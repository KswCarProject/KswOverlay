...
"%cd%\.compiler\aapt.exe" p -S "%cd%\resources\res-mtk" -M "%cd%\.compiler\manifest\MTK\AndroidManifest.xml" -I "%cd%\.compiler\framework-res-mtk.apk" -F kswoverlay-mtk.apk -f

echo Signing overlay APK...
"%cd%\.compiler\apksigner.bat" sign --ks "%cd%\.compiler\overlaysig.jks" --ks-pass pass:nicholaschum --key-pass pass:nicholaschum kswoverlay-mtk.apk